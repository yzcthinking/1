...
constexpr uint8_t TFT_TEXT_SIZE = 10;
...
void showTftMessage(const String& message)
{
  ...
  const int16_t characterWidth = 6 * TFT_TEXT_SIZE;
  const int16_t charactersPerLine = tft.width() / characterWidth;
  int16_t x = 0;
  int16_t y = 0;

  for (size_t index = 0; index < message.length(); ++index) {
    if (x + characterWidth > tft.width()) {
      x = 0;
      y += 8 * TFT_TEXT_SIZE;
    }
    if (y + 8 * TFT_TEXT_SIZE > tft.height()) {
      break;
    }
    tft.setCursor(x, y);
    tft.write(message[index]);
    x += characterWidth;
  }
}
...
void handleQR()
{
  ...
  const String barcode = server.arg("plain");
  Serial.print("Barcode: ");
  Serial.println(barcode);
  showTftMessage(barcode);
