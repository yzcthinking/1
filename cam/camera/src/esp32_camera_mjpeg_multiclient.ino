...
<head>
  ...
  <style>
    ...
    img { display: block; width: 100%; height: auto; border-radius: 6px; background: #222; }
    a { color: #8fd3ff; }
  </style>
</head>
<body>
  <main>
    <h1>ESP32-CAM</h1>
    <img src="/mjpeg/1" alt="Live camera stream">
    <p><a href="/jpg">Capture JPEG</a> | <a href="/quagga">Barcode Scanner</a></p>
  </main>
</body>
</html>
)rawliteral";

const char QUAGGA_HTML[] PROGMEM = R"rawliteral(
<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
...
<body>
  <main>
    ...
    <img id="stream" src="/mjpeg/1" alt="ESP32-CAM stream">
    <canvas id="snapshot"></canvas>
    <div id="result">Waiting for a barcode...</div>
    ...
    function scanFrame() {
      ...
      Quagga.decodeSingle({
        ...
        if (code && code !== lastCode) {
          ...
          fetch('/qr', { method: 'POST', body: code });
        }
      });
    }
...
void mjpegCB(void* pvParameters) {
  ...
  server.on("/quagga", HTTP_GET, []() {
    server.send_P(200, "text/html", QUAGGA_HTML);
  });
...
bool isProductCode(const String& value)
{
...
void handleQR()
{
  if (server.hasArg("plain")) {
    String qrData = server.arg("plain");

    if (!isProductCode(qrData)) {
      server.send(400, "text/plain", "Expected DDD+DDD+DDD+DDD");
      return;
    }
...
void setup()
{
  ...
  // Create a direct Wi‑Fi network for just the ESP32-CAM and a laptop/PC.
  // No router, phone hotspot, or other device is required.
  IPAddress ip;
  const char* apName = "ESP32-CAM";
