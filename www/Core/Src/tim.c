...
static void InitServoTimer(TIM_HandleTypeDef *timer, TIM_TypeDef *instance)
{
  TIM_MasterConfigTypeDef master = {0};
  TIM_OC_InitTypeDef output_compare = {0};

  timer->Instance = instance;
  timer->Init.Prescaler = 83U;
  timer->Init.CounterMode = TIM_COUNTERMODE_UP;
  timer->Init.Period = 19999U;
  ...
  if (HAL_TIM_PWM_ConfigChannel(timer, &output_compare, TIM_CHANNEL_1) != HAL_OK) Error_Handler();
  HAL_TIM_MspPostInit(timer);
}

void MX_TIM3_Init(void)
{
  InitServoTimer(&htim3, TIM3);
}

void MX_TIM4_Init(void)
{
  InitServoTimer(&htim4, TIM4);
}

void MX_TIM5_Init(void)
{
  InitServoTimer(&htim5, TIM5);
}
/* TIM2 init function */
